...
int isearch_key(gtcaca_editor_widget_t *ed, int key)
{
  ...
  case CACA_KEY_DELETE:
    ...
    return 1;
  default: {
    char u[8]; int ulen = key_to_utf8(key, u);        /* accented input → UTF-8 bytes */
    int one = (key >= 32 && key <= 126);
    if (ulen || one) {
      if (!ulen) { u[0] = (char)key; u[1] = '\0'; ulen = 1; }
      if (g_isearch_qlen + ulen < (int)sizeof g_isearch_query) {
        memcpy(g_isearch_query + g_isearch_qlen, u, (size_t)ulen);
        g_isearch_qlen += ulen; g_isearch_query[g_isearch_qlen] = '\0';
...
static const char *g_mx_commands[] = { "help", "diagram", "undo", "redo",
                                       "snake", "sokoban", "bird", "wordguess",
                                       "wordguess-wild", "describe-bindings",
...
int minibuffer_key(int key)
{
  ...
  case CACA_KEY_LEFT:  case 2 /* C-b */:                            /* step over a whole UTF-8 char */
    ...
    mb_status(); return 1;
  case CACA_KEY_RIGHT: case 6 /* C-f */:
    if (g_mb_point < g_mb_len) { g_mb_point++; while (g_mb_point < g_mb_len && ((unsigned char)g_mb_buf[g_mb_point] & 0xC0) == 0x80) g_mb_point++; }
    mb_status(); return 1;
  ...
  case 4 /* C-d */:                                                /* delete whole char at point */
    if (g_mb_point < g_mb_len) {
      ...
      memmove(g_mb_buf + g_mb_point, g_mb_buf + end, (size_t)(g_mb_len - end) + 1);
      g_mb_len -= end - g_mb_point;
    }
    mb_status(); return 1;
  default: {
    char u[8]; int ulen = key_to_utf8(key, u);        /* accented input → UTF-8 bytes */
    int one = (key >= 32 && key <= 126);
    if ((ulen || one)) {
      if (!ulen) { u[0] = (char)key; ulen = 1; }
      if (g_mb_len + ulen < (int)sizeof g_mb_buf) {    /* insert at point */
        memmove(g_mb_buf + g_mb_point + ulen, g_mb_buf + g_mb_point, (size_t)(g_mb_len - g_mb_point) + 1);
        memcpy(g_mb_buf + g_mb_point, u, (size_t)ulen);
        g_mb_len += ulen; g_mb_point += ulen;
      }
    }
    mb_status();
    return 1;
...
void mx_done(const char *cmd)
{
  if (!strcmp(cmd, "help") || !strcmp(cmd, "describe-bindings") || !strcmp(cmd, "?"))
    ...
    run_bird();
  else if (!strcmp(cmd, "wordguess") || !strcmp(cmd, "wordle"))
    run_wordguess(0);
  else if (!strcmp(cmd, "wordguess-wild"))
    run_wordguess(1);
